...
public class ParseTree 
{
	...
	private void parse() throws ParsingException
	{
		...
		while(parsed.size()>1)
		{
			for (int x=0; x<=parsed.size()-2; x++)
			{
				...
				if (node==null) throw new ParsingException("Input string is not contained in Grammar");
	...
	private Typology getRoute(ParsingNode node)
	{
		for (ParsingNode n: node.getSons())
		{
			switch (n.getValue())
			{
				case WH: return Typology.QUESTION;
				case GRE: return Typology.AFFIRMATION;
				case YN: return Typology.AFFIRMATION;
				default: getRoute(n); 
			}
		}
